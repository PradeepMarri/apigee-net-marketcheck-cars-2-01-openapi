...
func Get_search_heavy_equipment_activeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["hours_used_range"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("hours_used_range=%v", val))
		}
		...
		url := fmt.Sprintf("%s/search/heavy-equipment/active%s", cfg.BaseURL, queryString)
		...
		var result models.HeavyEquipmentsSearchResponse
...
func CreateGet_search_heavy_equipment_activeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_heavy-equipment_active",
		mcp.WithDescription("Gets active heavy equipment listings for the given search criteria"),
		...
		mcp.WithString("category", mcp.Description("To filter heavy equipments on their category")),
		mcp.WithString("sub_category", mcp.Description("To filter heavy equipments on their sub-category")),
		mcp.WithString("hours_used_range", mcp.Description("Hours used range to filter heavy equipments with the their usage in the range given. Range to be given in the format - min-max e.g. 1000-5000")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_search_heavy_equipment_activeHandler(cfg),
	}
}
