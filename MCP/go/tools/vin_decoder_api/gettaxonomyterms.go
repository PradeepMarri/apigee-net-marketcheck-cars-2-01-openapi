...
func GettaxonomytermsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/specs/car/terms%s", cfg.BaseURL, queryString)
...
func CreateGettaxonomytermsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_specs_car_terms",
		mcp.WithDescription("API for getting terms from taxonomy"),
		mcp.WithString("api_key", mcp.Description("The API Authentication Key. Mandatory with all API calls.")),
		mcp.WithString("field", mcp.Required(), mcp.Description("Comma separated list of fields to get terms for")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GettaxonomytermsHandler(cfg),
	}
}
