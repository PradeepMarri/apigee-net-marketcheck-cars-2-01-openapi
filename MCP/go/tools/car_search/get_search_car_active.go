...
func Get_search_car_activeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["vins"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("vins=%v", val))
		}
		if val, ok := args["taxonomy_vins"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("taxonomy_vins=%v", val))
		}
		...
		if val, ok := args["high_value_features"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("high_value_features=%v", val))
		}
		...
		url := fmt.Sprintf("%s/search/car/active%s", cfg.BaseURL, queryString)
...
func CreateGet_search_car_activeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_car_active",
		mcp.WithDescription("Gets active car listings for the given search criteria"),
		...
		mcp.WithString("vins", mcp.Description("Comma separated list of 17 digit vins to search the matching cars for. Only 10 VINs allowed per request. If the request contains more than 10 VINs the first 10 VINs will be considered. Could be used as a More Like This or Similar Vehicles search for the given VINs. Ths vins parameter is an alternative to taxonomy_vins or ymmt parameters available with the search API. vins and taxonomy_vins parameters could be used to filter our cars with the exact build represented by the vins or taxonomy_vins whereas ymmt is a top level filter that does not filter cars by the build attributes like doors, drivetrain, cylinders, body type, body subtype, vehicle type etc")),
		mcp.WithString("taxonomy_vins", mcp.Description("Comma separated list of 10 letters excert from the 17 letter VIN. The 10 letters to be picked up from the 17 letter VIN are - first 8 letters and the 10th and 11th letter. E.g. For a VIN - 1FTFW1EF3EKE57182 the taxonomy vin would be - 1FTFW1EFEK  A taxonomy VIN identified a build of a car and could be used to filter our cars of a particular build. This is an alternative to the vin or ymmt parameters to the search API.")),
		mcp.WithString("mm", mcp.Description("Make-Model concatenated string. To help passing the results of auto-complete API on mm field, use this parameter and pass in the selected value as is")),
		mcp.WithString("ymm", mcp.Description("Year-Make-Model concatenated string. To help passing the results of auto-complete API on ymm field, use this parameter and pass in the selected value as is")),
		...
		mcp.WithString("high_value_features", mcp.Description("To filter listings on their high_value_features. Results will be intersection of provided HVFs")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_search_car_activeHandler(cfg),
	}
}
