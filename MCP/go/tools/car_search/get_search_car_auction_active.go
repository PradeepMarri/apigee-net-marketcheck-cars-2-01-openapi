...
func Get_search_car_auction_activeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["title_type"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("title_type=%v", val))
		}
		...
		url := fmt.Sprintf("%s/search/car/auction/active%s", cfg.BaseURL, queryString)
...
func CreateGet_search_car_auction_activeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_car_auction_active",
		mcp.WithDescription("Gets active auction car listings for the given search criteria"),
		...
		mcp.WithString("title_type", mcp.Description("To filter on title type")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_search_car_auction_activeHandler(cfg),
	}
}
