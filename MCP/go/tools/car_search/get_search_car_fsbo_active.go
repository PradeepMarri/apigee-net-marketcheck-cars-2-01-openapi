...
func Get_search_car_fsbo_activeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["exclude_dealer_listings"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("exclude_dealer_listings=%v", val))
		}
		...
		url := fmt.Sprintf("%s/search/car/fsbo/active%s", cfg.BaseURL, queryString)
...
func CreateGet_search_car_fsbo_activeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_search_car_fsbo_active",
		mcp.WithDescription("Gets active private party car listings for the given search criteria"),
		...
		mcp.WithBoolean("exclude_dealer_listings", mcp.Description("A list of fsbo listings to exclude from result")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Get_search_car_fsbo_activeHandler(cfg),
	}
}
