...
func Get_dealer_heavy_equipment_idHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/dealer/heavy-equipment/%s%s", cfg.BaseURL, id, queryString)
...
func CreateGet_dealer_heavy_equipment_idTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_dealer_heavy-equipment_id",
		mcp.WithDescription("Dealer by id"),
		mcp.WithString("api_key", mcp.Description("The API Authentication Key. Mandatory with all API calls.")),
		mcp.WithString("id", mcp.Required(), mcp.Description("Dealer id to get all the dealer info attributes")),
		mcp.WithBoolean("provider", mcp.Description("boolean param to include site providers name in response")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_dealer_heavy_equipment_idHandler(cfg),
	}
}
