...
func CrmcheckHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["sale_date"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("sale_date=%v", val))
		}
		...
		url := fmt.Sprintf("%s/crm_check/car/%s%s", cfg.BaseURL, vin, queryString)
...
func CreateCrmcheckTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_crm_check_car_vin",
		mcp.WithDescription("CRM check of a particular vin"),
		...
		mcp.WithString("sale_date", mcp.Required(), mcp.Description("sale date to check whether after this listing has appeared or not. Must be 8 character long, with YYYYMMDD format")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    CrmcheckHandler(cfg),
	}
}
